...
type mockFileblockCreator[O cmp.Ordered, E db.Entry[O]] struct {
	...
	newFileblock      func(es db.Entries[O, E], builder *db.MetadataBuilder[O]) error
}

func (m *mockFileblockCreator[O, E]) NewFileblock(es db.Entries[O, E], builder *db.MetadataBuilder[O]) error {
	m.newFileblockCount++
...
func TestInMemoryWalFlushStrategy(t *testing.T) {
	...
	t.Run("ItemLimitFlushStrategy", func(t *testing.T) {
		...
		fbcreator.newFileblock = func(es db.Entries[int32, *db.Kv], builder *db.MetadataBuilder[int32]) error {
			require.Equal(t, 2, es.SecondaryIndicesLen())
			require.Equal(t, 2, len(es.Get("wal_cpu").Val))
			return nil
		}
		wal.Append(db.NewKv("wal_cpu", "wal_instance1", []int32{1, 2}))
		wal.Append(db.NewKv("wal_mem", "wal_instance1", []int32{3, 4}))
		...
		wal.Append(db.NewKv("wal_cpu", "wal_instance1", []int32{5, 6, 7, 8, 9}))
	...
	t.Run("SizeLimitWalFlushStrategy", func(t *testing.T) {
		...
		wal.Append(db.NewKv("wal_cpu", "wal_instance1", []int32{1, 2}))
		wal.Append(db.NewKv("wal_cpu", "wal_instance1", []int32{3, 4}))
